...
(setq shortList (make-array 10 :adjustable t :fill-pointer 0)) ;Lisp strings are represented as vectors of characters
...
(defun tokenize (string)
  (loop
     ...
     for space = (position #\space string :start start)  ;This tokenizes a file by whitespace and returns a list;
     for token = (subseq string start space)
     unless (string= token "") collect token
     until (not space)))

(defparameter *stringList*
   (with-open-file (stream filename :direction :input) ;Returning list
...
(setq b (ignore-errors (parse-number token))) ;Suppose to check if string is numeric and remove it
(if (not b)
   (progn
     (if (>  (length token) num)
	...
	(setq longList tmpList)
     ...
     (setq tmpList (make-array 0 :adjustable t :fill-pointer 0)) ;Emptying out vector
     (terpri)
     (incf lineIndex 1)
     (format t "~8,d" lineIndex)
     (format t "~d" "  ")
     ...
     (vector-push-extend token tmpList) ;Appending to string
